...
/**
 * Analisa texto consolidado usando o modelo Gemini (apenas texto)
 * @param {string} textoConsolidado - Texto consolidado de múltiplos PDFs
 * @param {string} modeloFicha - Modelo de ficha técnica (opcional)
 * @returns {Object} - Objeto com as informações extraídas e formatadas
 */
export async function analisarTextoConsolidado(textoConsolidado, modeloFicha = null) {
  try {
    // Prompt para retornar JSON estruturado em seções
    let prompt = `Analise o texto consolidado abaixo (que contém dados de múltiplos projetos) e extraia as informações para preencher uma ficha técnica consolidada. Retorne um JSON com as seguintes seções e campos. Se algum campo não estiver presente, use "FALTANDO INFORMAÇÃO".
...
`;
    ...
    // Chama o modelo Gemini apenas com texto (sem arquivo)
    const response = await ai.models.generateContent({
      model: "gemini-2.0-flash",
      contents: [
        {
    ...
    console.error("Erro ao analisar texto consolidado com Gemini:", error);
    throw new Error(`Falha na análise com IA: ${error.message}`);
  }
}